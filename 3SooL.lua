...
local Settings = {
    Aimbot = { 
        Enabled = false, ShowFOV = false, FOV_Radius = 120,
        TriggerBot = false, -- إطلاق تلقائي
        HitboxExpander = false, HitboxSize = 25, -- تكبير الجسم
        WallBang = false -- طلقة مخترقة للجدران
    },
    ESP = { 
        Enabled = false, 
        ShowNames = false, -- إظهار الاسم والمسافة
        Chams = false -- إضاءة الخصوم
    },
    Combo = { 
        SpeedBoost = false, SpeedMultiplier = 2, 
        InfiniteJump = false, Noclip = false, -- اختراق الجدران
        GodMode = false, InfAmmo = false,
        JumpPowerBoost = false, JumpPowerMultiplier = 2 -- قوة القفزة
    },
...
local function MakeDraggable(gui)
    ...
    UserInputService.InputChanged:Connect(function(input)
        if input == dragInput and dragging then
            if input.UserInputState == Enum.UserInputState.End then dragging = false else
...
local function CreateTabButton(text, targetScroll)
    ...
    btn.MouseButton1Click:Connect(function()
        AimbotScroll.Visible = (targetScroll == AimbotScroll); ESPScroll.Visible = (targetScroll == ESPScroll); ComboScroll.Visible = (targetScroll == ComboScroll); FriendsScroll.Visible = (targetScroll == FriendsScroll)
...
CreateTabButton("Aimbot", AimbotScroll); CreateTabButton("ESP", ESPScroll); CreateTabButton("Combo", ComboScroll); CreateTabButton("Friends", FriendsScroll)
...
local function CreateInputItem(parentScroll, text, placeholder, callback)
    ...
    box.FocusLost:Connect(function() local num = tonumber(box.Text); if num then callback(num) else box.Text = placeholder end end)
end
                ...
                if part then
                    ...
                    if onScreen then
                        ...
                        if dist <= shortestDist then
                            -- مع WallBang، نتجاهل فحص الجدران
...
RunService:BindToRenderStep("MobileAimbotCore", Enum.RenderPriority.Camera.Value + 1, function()
    ...
    if Settings.Aimbot.TriggerBot and targetPart then
        ...
        if triggerTick % 10 == 0 then -- لإعطاء مهلة بين كل ضغطة والثانية لعدم التسبب بانهيار اللعبة
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") and not Settings.Friends[p.UserId] then
            ...
            if Settings.Aimbot.HitboxExpander then
                ...
                hrp.Size = Vector3.new(2, 2, 1) -- الحجم الطبيعي
                hrp.Transparency = 1
            end
        end
    end
...
FloatingTpBtn.MouseButton1Click:Connect(function()
    ...
    if nearest then myRoot.CFrame = nearest.Character.HumanoidRootPart.CFrame * CFrame.new(0, 0, 3) end
end)
